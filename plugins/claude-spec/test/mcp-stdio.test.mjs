...
async function workspace({ withAdapter = true } = {}) {
  const root = await mkdtemp(path.join(os.tmpdir(), 'claude-spec-stdio-'));
...
test('stdio MCP 可用 CLAUDE_SPEC_PROJECT_ROOT 脱离进程 cwd 定位项目', async () => {
  const projectRoot = await workspace();
  const pluginRoot = path.resolve(import.meta.dirname, '..');
  const server = spawn(process.execPath, [path.join(pluginRoot, 'mcp-server.mjs')], {
    cwd: pluginRoot,
    env: { ...process.env, CLAUDE_SPEC_PROJECT_ROOT: projectRoot }
  });
  const replies = [];
  let pending = '';
  server.stdout.setEncoding('utf8');
...
test('stdio MCP 拓扑默认 local，可用 CLAUDE_SPEC_TOPOLOGY=cowork 显式切换', async () => {
  const projectRoot = await workspace();
  const pluginRoot = path.resolve(import.meta.dirname, '..');

  const callHealth = async (env) => {
    const server = spawn(process.execPath, [path.join(pluginRoot, 'mcp-server.mjs')], { cwd: pluginRoot, env });
    const replies = [];
    let pending = '';
    server.stdout.setEncoding('utf8');
    server.stdout.on('data', (chunk) => { pending += chunk; const lines = pending.split('\n'); pending = lines.pop(); for (const line of lines) if (line) replies.push(JSON.parse(line)); });
    try {
      server.stdin.write(`${JSON.stringify({ jsonrpc: '2.0', id: 1, method: 'tools/call', params: { name: 'spec_health', arguments: { projectRoot } } })}\n`);
      await waitForReplies(replies, 1);
      return replies[0].result.structuredContent.gate.topology;
    } finally { server.kill(); }
  };

  // 显式去掉 `CLAUDE_SPEC_TOPOLOGY`，不直接传 `process.env`：这条测试断言的是「不设时的
  // default」，如果跑测试的这台机器 / shell 里已经为了手测 Cowork 模式导出过这个变量，
  // 直接传 `process.env` 会把「没设」和「外部环境凑巧设成了别的值」混为一谈，产生假失败。
  const { CLAUDE_SPEC_TOPOLOGY: _ambientTopology, ...envWithoutTopology } = process.env;

  assert.equal(await callHealth(envWithoutTopology), 'local', '不设 CLAUDE_SPEC_TOPOLOGY 时，今天才发现之前一直默认按 Cowork 心智在跑——default 必须是 local');
  assert.equal(await callHealth({ ...envWithoutTopology, CLAUDE_SPEC_TOPOLOGY: 'cowork' }), 'cowork');
});

test('stdio MCP 遇到非法 CLAUDE_SPEC_TOPOLOGY 直接快速失败退出，不静默兜底成 local', async () => {
  const pluginRoot = path.resolve(import.meta.dirname, '..');
  const server = spawn(process.execPath, [path.join(pluginRoot, 'mcp-server.mjs')], {
    cwd: pluginRoot,
    env: { ...process.env, CLAUDE_SPEC_TOPOLOGY: 'desktop' },
  });
  let stderr = '';
  ...
  try {
    const [code] = await Promise.race([
      new Promise((resolve) => server.on('exit', (exitCode, signal) => resolve([exitCode, signal]))),
      new Promise((_, reject) => setTimeout(() => reject(new Error('server did not exit within 2s on invalid CLAUDE_SPEC_TOPOLOGY')), 2_000)),
    ]);
    assert.notEqual(code, 0, '非法拓扑值要让进程快速失败退出，而不是悄悄用 local 顶上');
    assert.match(stderr, /CLAUDE_SPEC_TOPOLOGY/);
  } finally { server.kill(); }
});
