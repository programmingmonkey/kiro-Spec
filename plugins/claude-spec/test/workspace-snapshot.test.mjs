...
test('Claude gate 的审计与心跳文件不能单独构成任务工作区变更证据', () => {
  const clean = canonicalWorkspaceSnapshot({ ...base, trackedDirty: [], untracked: [], platform: 'linux', eol: 'lf', untrackedPolicy: 'include' });
  const observabilityOnly = canonicalWorkspaceSnapshot({
    ...base,
    trackedDirty: [{ path: '.claude/claude-spec-gate.log', contentRevision: 'sha256:audit' }],
    untracked: [{ path: '.claude/claude-spec-gate.heartbeat', contentRevision: 'sha256:heartbeat' }],
    platform: 'linux', eol: 'lf', untrackedPolicy: 'include',
  });
  assert.equal(observabilityOnly.revision, clean.revision);
});
