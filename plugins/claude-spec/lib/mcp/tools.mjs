...
workspaceSnapshot.description = '由宿主采集的结构化工作区快照；路径集必须排除 .kiro/specs/**、.kiro-spec-private/**、.claude/claude-spec-gate.log 与 .claude/claude-spec-gate.heartbeat。服务端会 canonicalize 并重算 revision，但不执行 Git。';
...
// `workspaceSnapshot` 在 `spec_task_plan` / `spec_task_begin` / `spec_task_complete` 三处内联重复，
// 合计 4,143 字节 ≈ 整个 spec 工具面的 20.9%。看上去是一块显眼的去重空间，**实测不可回收**：
//
// 2026-09-16 实现并测量过 `$defs` + `$ref` 的去重版（`CLAUDE_SPEC_SCHEMA_REFS` 开关）：
// MCP 的 `tools/list` 给每个工具一份**独立的 `inputSchema`**，`#/$defs/...` 只在该工具自己的
// schema 根内解析，够不到别的工具。于是三个工具各自还得带一份 `$defs`，再加三个 `$ref` 的间接层 ——
// 实测 **净增 201 字节**（全量 19,805 → 20,006），比内联更差。那一版连同它的开关已经撤掉。
//
// 剩下的两条路都要拿准确性换，所以都不走：
//   · 把 schema 压成 `{type:'object'}` 加一段散文 —— 模型得猜快照的形状，服务端也不再逐字段校验；
//   · 让 begin/complete 复用 plan 的快照 —— 那正好抹掉「计划与开工之间工作区漂移了没有」这条判据。
//
// 结论：这 20.9% 是**内在成本**，不是浪费。留这段注释是为了让下一个看见它的人不用再测一遍。
export const tools = [
  define('spec_health', '检查目标项目 adapter、写入边界和 claude-spec 基础能力。', object({ projectRoot }, ['projectRoot']), { readOnly: true }),
  ...
  define('spec_read', '读取一个已管理 artifact，并刷新其 rawRevision 与语义指纹基线。默认回全文；outline=true 只回小节目录（标题/行号/字符数），section="<标题>" 只回该小节正文。后两者是部分读，返回 partial:true —— 不要拿它的 rawRevision 去整份覆写。', object({ projectRoot, spec, artifact, outline: { type: 'boolean' }, section: { type: 'string', minLength: 1, description: '顶层 ## 小节标题，需逐字匹配（先用 outline 取标题）。' } }, ['projectRoot', 'spec', 'artifact'])),
  define('spec_context', '加载写入或执行当前 artifact 所需的项目权威规则，并签发短期 contextProof。已持有正文时用 knownRevisions 回传上次拿到的 rawRevision：逐字节未变的文件只回执不回正文（unchanged:true），变了的照常回全文。', object({ projectRoot, spec, artifact, knownRevisions: { type: 'array', description: '上次本工具返回的 {path, rawRevision}；仅用于跳过未变文件的正文。', items: object({ path: { type: 'string', minLength: 1 }, rawRevision: revision }, ['path', 'rawRevision']) } }, ['projectRoot', 'spec', 'artifact']), { readOnly: true }),
  define('spec_write', '使用 contextProof 与 rawRevision CAS 原子替换当前阶段的 Spec Markdown。传 signature 时，§4.3.2 署名会在**同一次原子写**里并进 content——署名与内容一起落盘，少一类事后操作，环境标识由插件校验；事后手工追加一行不受这份担保。', object({ projectRoot, spec, artifact, content: { type: 'string' }, expectedRawRevision: nullableRevision, contextProof: { type: 'string', minLength: 1 }, signature: { type: 'string', minLength: 1, description: '本次改了什么（§4.3.2 署名的说明段）。省略则不写署名，返回 attributionWarning。' } }, ['projectRoot', 'spec', 'artifact', 'content', 'expectedRawRevision', 'contextProof']), { destructive: true }),
  define('spec_amend', '已批准 Spec 的增量修正通道，改一处不必重发整份正文：param 用 from/to 就地改参数值（from 须恰好命中一次）；requirement 追加需求，编号续在现有最大值之后；design 追加 ## Amendments 条目并在 anchor 行后插入 pointer —— `heading`（或等价的 `title`，二选一）是**这一条修正自己的**标题（`### …` 或裸文本），**不要**传 section 名 `## Amendments`：该段由工具自建，传它会让文档出现两个同名二级标题。拒改任务体，拒绝 _archive/ 下的 Spec，写入必定作废该 artifact 及其下游审批。', object({ projectRoot, spec, kind: { type: 'string', enum: ['param', 'requirement', 'design'] }, file: artifact, from: markdown, to: markdown, title: markdown, body: markdown, heading: markdown, anchor: markdown, pointer: markdown, expectedRawRevision: nullableRevision, contextProof: { type: 'string', minLength: 1 }, signature: { type: 'string', minLength: 1, description: '本次改了什么（§4.3.2 署名）。' } }, ['projectRoot', 'spec', 'kind', 'contextProof']), { destructive: true }),
