...
test('semantic-v1 忽略合法 event block 的位置与空白表示，但不忽略正文', async () => {
...
test('semantic-v1 不会把代码围栏内的伪任务 checkbox 当作可忽略状态', async () => {
...
test('semantic-v1 保留缩进代码围栏内的伪任务 checkbox', () => {
  const pending = '   ```md\n- [ ] 1.1 Documentation\n   ```\n';
  const done = pending.replace('[ ]', '[x]');
  assert.notEqual(
    computeApprovalFingerprint({ artifact: 'tasks', markdown: pending }),
...
test('semantic-v1 不会让四空格缩进代码块吞掉后续任务语义', () => {
  const baseline = '    ```md\n- [ ] 1. Real task\n';
...
test('semantic-v1 保留任务列表容器内四空格围栏示例的 checkbox', () => {
...
test('semantic-v1 保留深层任务回退到父级容器后的围栏示例 checkbox', () => {
...
test('semantic-v1 不让未闭合的缩进围栏候选吞掉后续任务语义', () => {
...
test('semantic-v1 不让缩进围栏候选跨越更浅任务吞掉语义', () => {
  const pending = '- [ ] 1. First task\n    ```md\n- [ ] 2. Real task\n```json\n{}\n```\n';
  const retitled = pending.replace('Real task', 'Different task');
  assert.notEqual(
    computeApprovalFingerprint({ artifact: 'tasks', markdown: pending }),
...
test('semantic-v1 保留正文段落边界和嵌套围栏中的 JSON 表示', () => {
...
test('semantic-v1 不把顶级缩进代码块中的伪任务当作合法 checkbox', () => {
...
test('semantic-v1 对等价 waves JSON 格式稳定', async () => {
